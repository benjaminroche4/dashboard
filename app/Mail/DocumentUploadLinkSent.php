<?php

declare(strict_types=1);

namespace App\Mail;

use App\Models\DocumentRequest;
use Illuminate\Bus\Queueable;
use Illuminate\Mail\Mailable;
use Illuminate\Mail\Mailables\Content;
use Illuminate\Mail\Mailables\Envelope;
use Illuminate\Queue\SerializesModels;

/**
 * E-mail au client avec le lien public de dépôt de ses pièces et son code
 * d'appairage, dans la langue de la liste (`Mail::to()->locale()`).
 */
final class DocumentUploadLinkSent extends Mailable
{
    use Queueable, SerializesModels;

    public function __construct(public readonly DocumentRequest $request) {}

    public function envelope(): Envelope
    {
        return new Envelope(subject: __(':name, vos pièces à déposer', ['name' => $this->request->first_name]).' · '.config('company.name'));
    }

    public function content(): Content
    {
        return new Content(view: 'emails.documents.upload-link', with: [
            'request' => $this->request,
            'url' => $this->request->publicUrl(),
            'company' => config('company'),
        ]);
    }
}
