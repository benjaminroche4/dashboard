            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0" role="presentation" align="center" style="max-width:600px;width:100%;padding:40px 20px;border:1px solid #e2e2e2;border-radius:12px;background-color:#ffffff">
                <tbody><tr><td>
                    ...
                    <h2 style="margin:0;padding:1em 0 0;font-size:1.6em;line-height:1.4;font-weight:600">
                        Bonjour {{ $firstName }}, un lead vous attend depuis {{ $minutes }} minutes
                    </h2>

                    <p style="margin:0;padding:0.6em 0 0;color:#525252">
                        {{ $lead->fullName() }} vous est attribué depuis le {{ $lead->created_at?->timezone('Europe/Paris')->translatedFormat('j F à H\hi') }} et n'a encore reçu aucun premier contact. Un appel ou un message maintenant fait toute la différence.
                    </p>
                    ...
                    <p style="margin:0;padding:1.6em 0 0">
                        <a href="{{ route('leads.show', $lead) }}" style="display:inline-block;padding:10px 18px;border-radius:8px;background-color:#7f1d1d;color:#ffffff;font-weight:600;text-decoration:none">Ouvrir la fiche et contacter le lead</a>
                    </p>
                </td></tr></tbody>
            </table>
        </td></tr></tbody>
