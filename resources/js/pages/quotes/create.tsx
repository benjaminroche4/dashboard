...
type Props = {
    ...
    /** Numéro que recevra le devis à sa création (affiché dans l'aperçu). */
    nextNumber: string;
    defaults: {
        currency: Currency;
        vat_rate: number;
...
export default function QuotesCreate({
    ...
    const submit = (event: FormEvent) => {
        ...
        const found = validateQuoteForm(form.data);
            ...
            <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col px-4">
                ...
                <div className="grid gap-8 pb-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                    ...
                    <aside className="bg-sidebar rounded-xl border p-2 lg:sticky lg:top-4 lg:self-start">
                        <InvoicePreview
                            kind="quote"
                            form={quoteFormToInvoiceForm(form.data)}
                            company={company}
                            offers={offers}
                            number={nextNumber}
                        />
