...
- Toute route applicative est derrière le middleware `auth`. `/` redirige vers `login` ou `dashboard`. Seule exception : le webhook signé `POST /webhooks/rip/contact` (voir « Leads »).
